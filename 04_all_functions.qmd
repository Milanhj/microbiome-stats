...
# Input formulas as list(c(~ x1 + x2), c(~ x2 + x1)) or list(c(~x1))
...
# time_col: column index with time
# stop_col: final column index of metadata
# group column: permutation column index
# formulas: list of length 1 or more with regression formulas
...
```{r}
#| label: deseq2-minp-longitudinal
...
`rank_sum_minp(dat, time_col, group_col, start_col, B = 9999, select_vars = NULL, thresh = NULL)`
```{r}
#| label: ranksum-minp
...
rank_sum_min_p <- function(dat, time_col, group_col, select_vars = NULL,
